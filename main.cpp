...
//INICIALIZA AS VARIÁVEIS DOS INIMIGOS
int varInimigos()
{
...
// CONTROLES DO PLAYER 1/ DETECÇÃO DAS BORDAS
int MovePlayer1()
{
...
// CONTROLES DO PLAYER 2/ DESTECÇÃO DAS BORDAS
int MovePlayer2()
{
...
// DETECTA COLISÃO ENTRE JOGADOR/INIMIGO
int colisaoJog1()
{
	i = MaxInimigos - 1;
	noInimigos = 0;
...
// DETECTA COLISÃO ENTRE INIMIGO/CENARIO
int colisaoTerra()
{
	i = MaxInimigos - 1;
	noInimigos = 0;
...
int Cena1()
{
	...
	outtextxy(0, 640, (char *)"é anunciado. Os países se uniram para conter a");
	outtextxy(0, 670, (char *)"invasão alienígina e salvar a Terra.");
	delay(4000);
...
int CenaFinal()
{
	...
	while(status = 2)
	{
		...
		outtextxy(0, 600, (char *)"Após a vitória da raça humana sobre os alieníginas,");
		outtextxy(0, 620, (char *)"os países perceberam a força que eles possuíam unidos.");
		outtextxy(0, 640, (char *)"Após a invasão, a guerra teve seu fim.");
